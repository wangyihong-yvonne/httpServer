...
/**
 * A task to perform
 * @param taskid pointer-encoded task id
 */
void task(void *taskid){
	printf("Thread #%u working on task%d\n",
           (int)pthread_self(), (int)(intptr_t)taskid);
}
...
int main(){
	...
	int i;
	for (i=0; i<20; i++){
		thpool_add_work(thpool, task, (void*)(intptr_t)1);
		thpool_add_work(thpool, task, (void*)(intptr_t)2);
	};
